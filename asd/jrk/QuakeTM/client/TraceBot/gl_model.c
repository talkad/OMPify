...
//		else
		if (Q_strncmp(mt->name,"sky",3))
		{
			...
			tx->gl_texturenum = 0; //GL_LoadTexture (mt->name, tx->width, tx->height, (byte *)(tx+1), true, false);
...
void Mod_LoadTexinfo (lump_t *l)
{
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		{
			out->texture = 16; //GAJA STRIP r_notexture_mip;	// checkerboard texture
			out->flags = 0;
		}
		else
		{
			...
			if (!out->texture)
			{
				out->texture =  16; //GAJA STRIP r_notexture_mip; // texture not found
				out->flags = 0;
			}
		}
	}
...
void * Mod_LoadSpriteFrame (void * pin, mspriteframe_t **ppframe, int framenum)
{
	...
	*ppframe = pspriteframe;
	...
	pspriteframe->gl_texturenum = 0; //GL_LoadTexture (name, width, height, (byte *)(pinframe + 1), true, true);
