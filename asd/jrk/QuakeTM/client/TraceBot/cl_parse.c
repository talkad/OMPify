...
char *svc_strings[] =
{
	...
	"svc_setview",		// [short] entity number                                        // 5
	"svc_sound",			// <see code>
	...
	"svc_updatefrags",	// [byte] [short]                                               // 15
	"svc_clientdata",		// <shortbits + data>
	"svc_stopsound",		// <see code>
	"svc_updatecolors",	// [byte] [byte]
	"svc_particle",		// [vec3] <variable>
	"svc_damage",			// [byte] impact [byte] blood [vec3] from                   // 20
...
void PrintParse(int code);

extern cvar_t  nodownload;  // GAJA: dissable downloading of files from the server
...
qboolean	CL_CheckOrDownloadFile (char *filename)
{
	...
	// GAJA: dissable downloading of files
	if (nodownload.value==1) 
	{
...
void CL_ParseStaticSound (void)
{
	...
	// STRIP S_StaticSound (cl.sound_precache[sound_num], org, vol, atten);
}
...
void CL_ParseStartSoundPacket(void)
{
    ...
    // STRIP S_StartSound (ent, channel, cl.sound_precache[sound_num], pos, volume/255.0, attenuation);
}       
...
#define SHOWNET(x) printf ("PARSE: readcount: %3i\t%s\n", msg_readcount-1, x);
/*
=====================
...
void CL_ParseServerMessage (void)
{
	...
	char		*s;
	int			i, j;
	double		s_frametime; // GAJA: temporary variable for reading float part of message for average server frame time
...
//
	...
	// other commands
		...
		case svc_print:
			...
			if (i == PRINT_CHAT)
			{
				// STRIP S_LocalSound ("misc/talk.wav");
				con_ormask = 128;
			}
		...
		case svc_serverdata:
			...
			CL_ParseServerData ();
			//GAJA STRIP vid.recalc_refdef = true;	// leave full screen intermission
			break;
		...
		case svc_cdtrack:
			...
			// STRIP CDAudio_Play ((byte)cl.cdtrack, true);
			break;

		case svc_intermission:
			cl.intermission = 1;
			cl.completed_time = realtime;
			// STRIP vid.recalc_refdef = true;	// go to full screen
			for (i=0 ; i<3 ; i++)
...
//			break;
		...
		// GAJA
		case svc_frametime:
			s_frametime = (double) MSG_ReadFloat();
			if ( s_frametime > 0.0 )
			server_frametime = s_frametime;
			break;

		// GAJA
		case svc_spawnspotinfo:
			spawnspot = MSG_ReadLong();
			if (trace_state == prepare_trace) trace_state = start_trace;
			printf("SPAWN SPOT: %ld\n", spawnspot);
			break;
