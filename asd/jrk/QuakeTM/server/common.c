...
char			com_token[1024];
THREADPRIVATE(msg_readcount, msg_badread, com_token)
...
{
	LOCKPRINT("THREAD %d\tRemoveLink\t\tlink = %p\tlink->next = %p\tlink->prev = %p\n", GetThreadId(), l, l->next, l->prev);
	l->next->prev = l->prev;
	l->prev->next = l->next;
}
...
{
	LOCKPRINT("THREAD %d\tInsertLinkBefore\t\tlink = %p\tlink->next = %p\tlink->prev = %p\tbefore = %p\tbefore->prev = %p\n", GetThreadId(), l, l->next, l->prev, before, before->prev);
	l->next = before;
	l->prev = before->prev;
	l->prev->next = l;
	l->next->prev = l;
...
{
	// GAJA: STATIC - instead of static caller function provides local string
...
void MSG_ReadDeltaUsercmd (usercmd_t *from, usercmd_t *move)
{
	...
	Q_memcpy (move, from, sizeof(*move));
...
static char	thr_value[4*512];
int			valueindex;
THREADPRIVATE (thr_value, valueindex)
...
// 		o = value[valueindex];
		...
		*o = 0;
		...
		if (!Q_strcmp (key, pkey) )
			return (thr_value + valueindex * 512);
...
GAJA for debug purposes
Print the contest of the buffer as string
====================
*/
...
void PrintBuffer(byte *buf, int size)
{
    ...
    {
      x=buf[i];
      Q_sprintf(str, "%s%02x", str, x); 
      if (((i+1)%2) == 0) Q_sprintf(str, "%s ", str); 
    }
    Q_sprintf(str,"%s\n", str); 
    for (i=0;i<size;i++)
    {
