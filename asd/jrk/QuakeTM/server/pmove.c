...
int			waterlevel;
int			watertype;
THREADPRIVATE(onground, waterlevel, watertype)

playermove_t	pmove;
float			frametime;
vec3_t		forward, right, up;
THREADPRIVATE(pmove, frametime, forward, right, up)
...
void PM_GroundMove (void)
{
	vec3_t dest;    // GAJA removed ... start
	pmtrace_t	trace;
	vec3_t	original, originalvel, down, upward, downvel;
...
// press down the stepheight
	...
	VectorCopy (pmove.origin, upward);

	// decide which one went farther
	downdist = (down[0] - original[0])*(down[0] - original[0])
		+ (down[1] - original[1])*(down[1] - original[1]);
	updist = (upward[0] - original[0])*(upward[0] - original[0])
		+ (upward[1] - original[1])*(upward[1] - original[1]);
