...
THREADPRIVATE (onground, waterlevel, watertype, pmove, net_drop, host_client, msg_readcount, msg_badread, movevars)


#define PLAYTOUCH_EDICTS (MAX_EDICTS+7)/8
edict_t					*sv_player;
usercmd_t				cmnd;	// GAJA changed to cmnd to avoid ambiguos variable names (cmd)
byte 					playertouch[PLAYTOUCH_EDICTS];
vec3_t					pmove_mins, pmove_maxs;
THREADPRIVATE(sv_player, cmnd, playertouch, pmove_mins, pmove_maxs)
...
// GAJA declarations of intermodule functions with attribute added
TM_CALLABLE
void SV_New_f (void);
TM_CALLABLE
...
void SV_PreSpawn_f (void)
{
	...
	if (!buf) {
		// GAJA: lines commented to avoid checksums. Instead act as everything is ok
		//host_client->checksum = sv.worldmodel->checksum;
	...
	CRITICAL_MESSAGE(host_client->msg_lock, CLIENT_MSG_LOCK, host_client->clientId, {
		...
		SZ_Clear(&host_client->netchan.message);  // GAJA OVERFLOW: added this to avoid overflow
		// Debug the overflow case
		//if ((host_client->netchan.message.cursize + sv.signon_buffer_size[buf]) > host_client->netchan.message.maxsize)
		//{
		//	printf("\n\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
		//	printf("PRESPAWN OVERFLOW\n\n");
		//	fwrite( host_client->netchan.message.data, sizeof(byte), host_client->netchan.message.cursize, stdout);
		//	fflush(stdout);
		//	printf("\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
		//	fwrite( sv.signon_buffers[buf], sizeof(byte), sv.signon_buffer_size[buf], stdout);
		//	fflush(stdout);    
		//	printf("\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n\n");
		//}
	...
	CRITICAL_MESSAGE(host_client->msg_lock, CLIENT_MSG_LOCK, host_client->clientId, {
		...
		for (i=0 ; i<MAX_LIGHTSTYLES ; i++)
		{
			ClientReliableWrite_Begin (host_client, svc_lightstyle, 
				3 + (sv.lightstyles[i] ? Q_strlen(sv.lightstyles[i]) : 1));
...
void SV_SpawnSpectator (void)
{
	...
	{
		e = EDICT_NUM(i);
		if (!Q_strcmp(PR_GetString(e->v.classname), "info_player_start"))
		{
...
void SV_Begin_f (void)
{
	...
	else
	{
      ...
      (&pr_global_struct->parm1)[i] = host_client->spawn_parms[i];
		...
		// GAJA: send spawn spot number to the client
		if (host_client->edict->v.send_spawn_msg) {
			CRITICAL_MESSAGE(host_client->msg_lock, CLIENT_MSG_LOCK, host_client->clientId, {
				ClientReliableWrite_Begin(host_client, svc_spawnspotinfo, 5);
				ClientReliableWrite_Long(host_client, (long) host_client->edict->v.spawnspot);
			});
			host_client->edict->v.send_spawn_msg = 0.0;
		}
	}
...
void SV_BeginDownload_f(void)
{
	...
	{
		Con_Printf("Disabled in TM version of server\n");
		Sys_Printf("Client request for BeginDownload rejected because of TM\n");
		return;
	}
...
// hacked by zoid to allow more conrol over download
	...
	if (Q_strstr (name, "..") || !allow_download.value
		// leading dot is no good
		|| *name == '.' 
		// leading slash bad as well, must be in subdir
		|| *name == '/'
		// next up, skin check
		|| (Q_strncmp(name, "skins/", 6) == 0 && !allow_download_skins.value)
		// now models
		|| (Q_strncmp(name, "progs/", 6) == 0 && !allow_download_models.value)
		// now sounds
...
void SV_Say (qboolean team)
{
	...
	if (host_client->spectator && (!sv_spectalk.value || team))
		Q_sprintf (text, "[SPEC] %s: ", host_client->name); 
	else if (team)
		Q_sprintf (text, "(%s): ", host_client->name);  
	else {
		Q_sprintf (text, "%s: ", host_client->name);  
	}
	...
	for (j = 0, client = svs.clients; j < MAX_CLIENTS; j++, client++)
	{
		...
		if (team)
		{
			...
			if (host_client->spectator) {
				...
				if (Q_strcmp(t1, t2) || client->spectator)
...
void SV_TogglePause (const char *msg)
{
	...
	sv.paused = (sv.paused == true) ? false : true;		// turn ON/OFF
...
//	client_t *cl;
	...
	if (sv.paused)
		Q_sprintf (st, "%s paused the game\n", host_client->name);  
	else
		Q_sprintf (st, "%s unpaused the game\n", host_client->name);  
...
Set the client which spectator will track
=================
*/
TM_CALLABLE
void SV_PTrack_f (void)
...
void SV_SetInfo_f (void)
{
	...
	if (!Q_strcmp(Info_ValueForKey(host_client->userinfo, Cmd_Argv(1)), oldval))
...
// GAJA: Added for client respawning
TM_CALLABLE
void SV_Respawn_f(void)
{
	SZ_Clear (&host_client->netchan.message);
	SZ_Clear (&host_client->datagram);
	host_client->num_backbuf = 0;

	// GAJA: respawn the client
	pr_global_struct->time = sv.time;
	pr_global_struct->self = EDICT_TO_PROG(sv_player);
	PR_ExecuteProgram (pr_global_struct->TraceRespawn); // PutClientInServer
...
// issued by hand at client consoles	
	...
	{"download", SV_BeginDownload_f},	// GAJA Disabled in TM version
	{"nextdl", SV_NextDownload_f},		// GAJA Disabled in TM version

	{"ptrack", SV_PTrack_f},			//ZOID - used with autocam

	{"snap", SV_NoSnap_f},

	{"respawn", SV_Respawn_f},			// GAJA: added this to enable respawn of the client - hack
...
//vec3_t	pmove_mins, pmove_maxs;	// GAJA: moved to the threadprivate section in the beggining

/*
====================
AddLinksToPmove

Traverse the areanode tree and find touched entities. Add those entities to the pmove
The function only checks entities for a single areanode. In the end we go down the tree 
to search other nodes.
The list is traversed and for each object we check in all 3 coordinates. 
If the object is out of range we skip it, otherwise we include it in the pmove.
For solid objects we set the model from sv_models, for others we set it to NULL
====================
*/
TM_CALLABLE
void AddLinksToPmove ( areanode_t *node )
{
	...
	physent_t	*pe;
    // GAJA:
    qboolean    max_reached = false;

	pl = EDICT_TO_PROG(sv_player);

	LOCKPRINT("THREAD %d\tAddLinksToPmove\n", GetThreadId());
	...
	CRITICAL_AREA(node->areanode_lock, {
		...
		// touch linked edicts
		// traverse the entity list for the node
		for (l = node->solid_edicts.next ; l != &node->solid_edicts ; l = next)        
		{
			//while (1)
			//{
					//l = next;
					//if (l == &node->solid_edicts)
					//{
					//        break;
					//}
				...
				|| check->v.solid == SOLID_SLIDEBOX)
			{
				...
				// this entity may be affected so add it to pmove
				...
				// save the origin of this entity to pmove stucture
...
void SV_PreRunCmd(void)
{
	Q_memset(playertouch, 0, PLAYTOUCH_EDICTS * sizeof(byte));
}
...
// ------------------------------------------
    ...
    TRANSACTION_COARSE
		...
		if (!host_client->spectator)
		{
			...
			// GAJA: send spawn spot number to the client
			if ((host_client->edict->v.send_spawn_msg) != 0.0) 
			{
				CRITICAL_MESSAGE(host_client->msg_lock, CLIENT_MSG_LOCK, host_client->clientId, {
					ClientReliableWrite_Begin(host_client, svc_spawnspotinfo, 5);
					ClientReliableWrite_Long(host_client, (long) host_client->edict->v.spawnspot);
		...
		pmove.cmd = *ucmd;
		pmove.dead = (sv_player->v.health <= 0) ? true : false;
		pmove.oldbuttons = host_client->oldbuttons;

    TRANSACTION_COARSE_END
    UNLOCK_COARSE
...
// ------------------------------------------
    ...
    LOCK_COARSE
    TRANSACTION_COARSE
	//LOCK_FINE(areanode_lock)
		AddLinksToPmove ( sv_areanodes );
	//UNLOCK_FINE(areanode_lock)
    TRANSACTION_COARSE_END
    UNLOCK_COARSE
...
//	seq_hash = (cl->netchan.incoming_sequence & 0xffff) ; // ^ QW_CHECK_HASH;
        ...
        while (1)
	{
		...
		//DEBUGPRINT(READ_PACKETS,HIGH) printf("GAJA: Character read in ExecuteMessage: \t%d\n", c);
		...
		default:
            LOCK_COARSE
            TRANSACTION_COARSE
		        DEBUGPRINT(READ_PACKETS,HIGH) 
		        {
			        PRINT("WRONG CHARACTER: %d = %c\n", c, c);    // GAJA:
			        PrintBuffer(net_message.data, net_message.cursize);
		        }
		...
		case clc_nop:
			//DEBUGPRINT(READ_PACKETS,HIGH) printf("MSGREAD: clc_nop\n");
			break;

		case clc_delta:
			//DEBUGPRINT(READ_PACKETS,HIGH) printf("MSGREAD: clc_delta\n");
			cl->delta_sequence = MSG_ReadByte ();
			break;

		case clc_move:
			//DEBUGPRINT(READ_PACKETS,HIGH) printf("MSGREAD: clc_move\n");
			if (move_issued)
			...
			// GAJA: commented this to be able to run traces
...
// 			}
		...
		case clc_stringcmd:	
			//DEBUGPRINT(READ_PACKETS,HIGH) printf("MSGREAD: clc_stringcmd\n");
	        MSG_ReadString (s, sizeof(s));
            LOCK_COARSE
            TRANSACTION_COARSE
				SV_ExecuteUserCommand (s);	// GAJA transactified
            TRANSACTION_COARSE_END
            UNLOCK_COARSE
			break;

		case clc_tmove:
			//DEBUGPRINT(READ_PACKETS,HIGH) printf("MSGREAD: clc_tmove\n");
			// teleport - only allowed to spectators                        
			o[0] = MSG_ReadCoord();
			o[1] = MSG_ReadCoord();
			o[2] = MSG_ReadCoord();
		...
		case clc_upload:
			//DEBUGPRINT(READ_PACKETS,HIGH) printf("MSGREAD: clc_upload\n");
			// GAJA 
			// Disabled in TM version of server
			// SV_NextUpload();
			Con_Printf("Disabled in TM version of server\n");
			Sys_Printf("Client request for NextUpload rejected because TM\n");
			break;
