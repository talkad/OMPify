...
A wrapper function for omp_get_thread_num()
================
*/
TM_PURE
...
//
	...
	/* GAJA:
			Select		Wait for events on file descriptors
				...
				If successful, select() returns the number of ready descriptors 
				that are contained in the bit masks. If the time limit expires, 
				select returns zero and sets errno to EINTR. On failure, it returns -1
				
			SelectText	Wait for event on typed ontyped file.
			GetFS		Return file selector
			FD_Clr		Clears a filedescriptor in a set
			FD_Set		Set a filedescriptor in a set
			FD_IsSet 	Check whether a filedescriptor is set
			
		These functions provide applications with  a  mechanism
		for  multiplexing  input/output  over a set of file descriptors.
		For each IO, functions track the given file descriptors for the event(s)			
	*/
		...
		if (select (net_socket+1, &fdset, NULL, NULL, &timeout) == -1)
			continue;
		stdin_ready = (FD_ISSET(0, &fdset) == 0) ? false : true;    // GAJA changed stdin_ready = FD_ISSET(0, &fdset);
