...
// 		SV_Error ("Entity number >= 512");
  // GAJA: changed to this
  if (to->number >= MAX_EDICTS)
    SV_Error ("Entity number >= %d", MAX_EDICTS);
...
void SV_WriteEntitiesToClient (client_t *client, sizebuf_t *msg)
{
	...
	// GAJA: The entire for loop was added for debug purposes
	DEBUGPRINT(GENERAL, HIGH)
	{
		for (i=0;i<MAX_PACKET_ENTITIES;i++)
		{
			...
			if (state->number > 512) 
			{
				PRINT("\n\nENTITY > 512 for i=%d\n and client %s. The value is %d\n", i, client->name, state->number);
				PRINT("sv.num_edicts = %d\n\n", sv.num_edicts);
			}
		}
	}
