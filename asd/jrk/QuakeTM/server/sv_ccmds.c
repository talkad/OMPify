...
// Buffer for redirected messages
...
extern redirect_t	sv_redirected;
THREADPRIVATE(host_client, sv_player, sv_redirected)
...
void SV_Snap (int uid)
{
	...
	memcpy(&cl->snap_from, &net_from, sizeof(netadr_t));	// GAJA: SIZEOF sizeof(net_from)
	if (sv_redirected != RD_NONE)
		cl->remote_snap = true;
	else
...
void SV_InitOperatorCommands (void)
{
	...
	Cmd_AddCommand ("logfile", SV_Logfile_f);			// TM unsafe
	Cmd_AddCommand ("fraglogfile", SV_Fraglogfile_f);	// TM unsafe

	Cmd_AddCommand ("snap", SV_Snap_f);					// TM unsafe
	Cmd_AddCommand ("snapall", SV_SnapAll_f);			// TM unsafe
	Cmd_AddCommand ("kick", SV_Kick_f);					// TM unsafe fclose
	Cmd_AddCommand ("status", SV_Status_f);

	Cmd_AddCommand ("map", SV_Map_f);					// TM unsafe
	Cmd_AddCommand ("setmaster", SV_SetMaster_f);

	Cmd_AddCommand ("say", SV_ConSay_f);
	Cmd_AddCommand ("heartbeat", SV_Heartbeat_f);
	Cmd_AddCommand ("quit", SV_Quit_f);					// TM unsafe
	Cmd_AddCommand ("god", SV_God_f);
	Cmd_AddCommand ("give", SV_Give_f);
	Cmd_AddCommand ("noclip", SV_Noclip_f);
	Cmd_AddCommand ("serverinfo", SV_Serverinfo_f);		// check if Z_free is TM safe
	Cmd_AddCommand ("localinfo", SV_Localinfo_f);
	Cmd_AddCommand ("user", SV_User_f);
	Cmd_AddCommand ("gamedir", SV_Gamedir_f);			// TM unsafe
