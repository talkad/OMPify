...
def extract_typedef_directives(curr_file_path, fake_header_path):
    '''
    Create fake header contains typedefs (without duplicates - with respect to the fake_header.h can be found in utils)

    Parameters:
        curr_file_path   - the file the typedefs will be extracted from
        fake_header_path - the path to the generated fake header
    '''

    general_typedefs = get_directives('path/to/file/utils')
    current_typedefs = get_directives(curr_file_path)
...
# may be useful some day (not now)
...
#     '''
#     Create fake headers contains typedefs in all repos
#     '''
#     typedefs_set = set()
